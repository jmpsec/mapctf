...
type DBManager struct {
	Conn   *gorm.DB
	Config *config.ConfigurationDB
	DSN    string
}

// LoadConfiguration to load the DB configuration file and assign to variables
func LoadConfiguration(file, key string) (config.ConfigurationDB, error) {
	var config config.ConfigurationDB
...
func PrepareDSN(config config.ConfigurationDB) string {
	switch config.Type {
	case DBTypePostgres:
		return fmt.Sprintf(
...
// CreateDBManager to initialize the DB struct
func CreateDBManager(dbConfig config.ConfigurationDB) (*DBManager, error) {
	db := &DBManager{}
	db.Config = &dbConfig
